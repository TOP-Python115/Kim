...
letters = set(s.translate(str.maketrans('', '', string.whitespace[:-1])))

# res = {letter: {len(word): s.count(word) for word in s.split() if letter in word} for letter in letters}

res = {c: {} for c in letters}

for word in s.split():
    for c in set(word):
        res[c][len(word)] = res[c].setdefault(len(word), 0) + word.count(c)
...
# {'p': {7: 1},
 ...
 # 'щ': {3: 19,
       ...
       # 13: 2,
       # 15: 1},
 # 'ъ': {5: 1, 6: 4, 7: 8, 8: 14, 9: 7, 10: 3, 18: 1},
 # 'ы': {2: 83,
       # 3: 36,
       # 4: 101,
       # 5: 168,
